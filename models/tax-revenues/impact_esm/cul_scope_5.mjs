...
export const income_tax = ({ taxpayer_table_in, taxpayer_id_in, band_in, usc_table_in }) => Math.max(paye({ taxpayer_table_in, taxpayer_id_in, band_in }) + prsi({ taxpayer_table_in, taxpayer_id_in }) - usc({ usc_table_in }) - tax_credit({}), 0);
...
export const usc = ({ usc_table_in }) => usc_table({ usc_table_in }).reduce((a, v) => a + usc_band_by_id({ band_id: v.band_id }), 0);
/*
